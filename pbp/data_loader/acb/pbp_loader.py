...
from pbp.resources.pbp.acb_pbp_item import ACBPbpItem
from web_loader import ACBWebLoader


class ACBPbpLoader(ACBFileLoader, ACBWebLoader):
    """
    Loads ACB Spanish League pbp data for game
    ...
    def _from_web(self):
        ...
        self.base_url = f'https://api2.acb.com/api/v1/openapilive/PlayByPlay/matchevents?idMatch={self.game_id}'
    ...
    def _make_pbp_items(self):
        self.items = [ACBPbpItem(item) for item in self.source_data if 'instant' not in item['type']['description'].lower()]
...
pbp_loader = ACBPbpLoader(game_id, 'web', competition)