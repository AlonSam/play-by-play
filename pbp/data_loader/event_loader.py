...
class EventLoader(object):
    """
    Base class for all EventLoaders.
    ...
    def pair_subs_at_current_time(self, idx):
        ...
        combined_subs = []
        period_start = sub.seconds_remaining == 600
        subs_in_to_remove = []
        for sub_in in subs_in:
            self_sub = self.get_self_sub(subs_out, sub_in.player_id)
            if self_sub:
                subs_in_to_remove.append(sub_in)
                subs_out.remove(self_sub)
        subs_in = [sub for sub in subs_in if sub not in subs_in_to_remove]
        if len(subs_in) != len(subs_out):
            if len(subs_in) - 5 == len(subs_out) and period_start:
                for sub_in in subs_in:
                    combined_subs.append(self.create_combined_sub(sub_in))
                return combined_subs, subs
            else:
                raise ValueError('Number of substitutions do not match')
        for i in range(len(subs_in)):
            combined_subs.append(self.create_combined_sub(subs_in[i], subs_out[i]))
        return combined_subs, subs
    ...
    def get_subs_at_current_time(self, idx, team_id):
        ...
        events = []
        while i < len(self.raw_events) and (
    ...
    @staticmethod
    def create_combined_sub(sub_in, sub_out=None):
        combined_sub = copy(sub_in)
    ...
    def get_previous_events_at_current_time(self, idx):
        ...
        events = []
        while i > 0 and (event.period == prev_event.period and event.seconds_remaining == prev_event.seconds_remaining):
